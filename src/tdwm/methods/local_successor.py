...
from tdwm.methods import successor_geometry as _successor_geometry


def successor_feature_basis(latent: torch.Tensor) -> torch.Tensor:
    """Backward-compatible export of the shared successor feature basis."""

    return _successor_geometry.successor_feature_basis(latent)


def goal_cost_weights(goal: torch.Tensor) -> torch.Tensor:
    """Backward-compatible export of the shared goal query weights."""

    return _successor_geometry.goal_cost_weights(goal)


def latent_goal_cost(latent: torch.Tensor, goal: torch.Tensor) -> torch.Tensor:
    """Backward-compatible export of the shared latent goal geometry."""

    return _successor_geometry.latent_goal_cost(latent, goal)


def successor_goal_cost(successor: torch.Tensor, goal: torch.Tensor) -> torch.Tensor:
    """Backward-compatible export of the shared successor goal geometry."""

    return _successor_geometry.successor_goal_cost(successor, goal)
