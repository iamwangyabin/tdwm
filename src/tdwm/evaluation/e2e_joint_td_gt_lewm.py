"""Controlled Cube evaluation for E2E Joint TD-GT-LeWM."""
...
def load_e2e_joint_td_gt_evaluation_protocol(
    path: str | Path,
) -> dict[str, Any]:
    with Path(path).open() as stream:
        protocol = yaml.safe_load(stream)
    validate_e2e_joint_td_gt_evaluation_protocol(protocol)
    return protocol


def validate_e2e_joint_td_gt_evaluation_protocol(
    protocol: dict[str, Any],
) -> None:
    if protocol.get("schema_version") != 1:
        raise ValueError("E2E Joint TD-GT-LeWM evaluation requires schema 1.")
    if protocol.get("method") != "e2e_joint_td_gt_lewm":
        raise ValueError("This evaluator only accepts E2E Joint TD-GT-LeWM.")
    if protocol.get("environment") != "cube" or protocol.get("stage") != "planner_evaluation":
        raise ValueError("E2E evaluation is locked to Cube planner evaluation.")
    if protocol.get("runtime", {}).get("stable_worldmodel_version") != "0.1.1":
        raise ValueError("E2E evaluation requires stable-worldmodel 0.1.1.")
    ...
    if planning["horizon"] != 5 or planning["action_block"] != planning["frame_skip"]:
        raise ValueError("Evaluation must use the training horizon and action block.")
    ...
    if planning["receding_horizon"] > planning["horizon"]:
        raise ValueError("CEM receding horizon cannot exceed its horizon.")
    ...
    if tail.get("objective") != "one_step_td":
        raise ValueError("E2E evaluation requires a one-step TD value.")
    if tail.get("training_input") != "lewm_predicted_terminal_history":
        raise ValueError("E2E evaluation requires the model-aware value.")
    if tail.get("history_size") != 3 or tail.get("latent_dim") != 192:
        raise ValueError("E2E evaluation requires history 3 and latent dim 192.")
    ...
    if tail.get("weight") != 1.0 or tail.get("upstream_scale_factor") != tail["latent_dim"]:
        raise ValueError("The E2E tail must use the locked planner scaling.")


def _validate_e2e_checkpoint(
    ...
    *,
    ...
    for key, expected_value in checks.items():
        if int(value_config.get(key, -1)) != int(expected_value):
            raise ValueError(f"E2E checkpoint {key} differs from protocol.")
    if not np.isclose(float(value_config["gamma"]), float(expected["gamma"])):
        raise ValueError("E2E checkpoint gamma differs from protocol.")
    if value_config.get("input_distribution") != expected["training_input"]:
        raise ValueError("E2E checkpoint input distribution differs from protocol.")
    if int(payload.get("epoch", -1)) != int(protocol["value_checkpoint"]["epoch"]):
        raise ValueError("E2E checkpoint epoch differs from protocol.")


def evaluate_e2e_joint_td_gt_lewm(
    *,
    ...
    return _evaluate_goal_tail_lewm(
        ...
        video=video,
        smoke=smoke,
        protocol_loader=load_e2e_joint_td_gt_evaluation_protocol,
        protocol_validator=validate_e2e_joint_td_gt_evaluation_protocol,
        value_loader=load_e2e_joint_td_goal_tail_value,
        value_validator=_validate_e2e_checkpoint,
        policy_builder=make_e2e_joint_td_goal_tail_policy,
