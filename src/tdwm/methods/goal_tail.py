...
The module deliberately contains only the additional mathematical object.  The
LeWM encoder, predictor, and SIGReg anti-collapse term remain owned by
``stable_worldmodel`` and the existing trainer.
"""
...
def goal_cost(latent: torch.Tensor, goal: torch.Tensor) -> torch.Tensor:
    """Return the mean squared latent distance to a goal.

    ``latent`` and ``goal`` may have arbitrary leading dimensions, but their
    final dimension must be the embedding dimension.
    """

    if latent.shape != goal.shape and latent.shape[-1] != goal.shape[-1]:
...
def discounted_goal_tail_target(
    ...
    *,
    ...
    """Build the normalized N-step discounted goal-tail target.

    ``future_latents`` has shape ``(..., N, D)``; ``goal`` and ``bootstrap``
    have shapes ``(..., D)`` and ``(...)`` respectively.  The leading
    dimensions are preserved, which lets the same function serve training and
    batched MPC candidate scoring.
    """
...
class GoalTailValue(nn.Module):
    ...
    The explicit ``[z, g, z-g]`` input makes the goal dependence visible and
    avoids introducing an action-conditioned successor representation.  The
    positive output matches the non-negative latent goal cost used by MPC.
    """
