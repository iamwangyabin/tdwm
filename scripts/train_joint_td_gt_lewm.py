...
from tdwm.training.joint_td_gt_lewm import train_joint_td_gt_lewm


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Jointly train LeWM dynamics and its model-aware TD goal tail."
    )
    parser.add_argument(
        "--config",
        default="configs/experiment/joint_td_gt_lewm_cube_train.yaml",
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--latent-cache-dir",
        default=None,
        help="Optional audited LeWM latent cache directory to reuse.",
    )
    ...
    parser.add_argument(
        "--resume-checkpoint",
        default=None,
        help="Resume the joint model, optimizer, scheduler, and RNG states.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
...
def main() -> None:
    ...
    if output_dir is None:
        output_dir = (
            ...
            / "joint_td_gt_lewm_cube_full"
        )
    result = train_joint_td_gt_lewm(
