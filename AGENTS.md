...
- 任何受版本控制的项目修改，包括代码、测试、配置和文档，都必须在完成验证后
  提交并推送到当前远程跟踪分支；推送成功前不得声称任务已经完成。
- 不得在仓库根目录随意创建脚本、配置、笔记、日志或中间文件。新文件必须进入
  本文规定的职责目录；确需新增根目录入口时，必须先确认其属于根目录白名单。
- 不得顺手重构、格式化、移动、重命名、删除或改写现有代码。修改必须直接服务于
  当前请求，保持最小范围，并保留用户未提交的改动和已有研究记录。
- 临时文件必须写入系统临时目录或仓库已忽略的 `tmp/`，不得散落在根目录、源码、
  配置、测试或报告目录中，也不得作为唯一副本长期保存。
...
仓库根目录只允许放置跨项目入口和仓库级元数据。允许的文件包括：

- `AGENTS.md`、`README.md`、`LICENSE`、`.gitignore`、`.gitattributes`；
- `pyproject.toml` 及其选定工具生成的唯一 lockfile；
- 由依赖事实来源生成的 `requirements.txt`；
- 主构建入口 `Dockerfile`、`.dockerignore`；有多个镜像时，其余 Dockerfile 放入
  `docker/`，不得继续堆在根目录；
- 经项目明确采用的单一仓库级工具入口，例如 `Makefile` 或 `.env.example`。

不在白名单中的新根目录文件必须先取得用户确认，并先更新本节说明其长期职责。
尤其禁止把临时 `.py`、`.sh`、`.yaml`、`.json`、notebook、Markdown 笔记、日志、
下载文件或实验输出直接放到根目录。不得为了规避本规则而创建含义模糊的新目录，
例如 `misc/`、`stuff/`、`new/` 或 `backup/`。
...
| 内容 | 位置 | 约束 |
| --- | --- | --- |
| 可导入的项目代码 | `src/tdwm/` | 按 `methods/`、`adapters/`、`training/`、`evaluation/` 的职责拆分 |
| 人工执行入口 | `scripts/` | 只做参数解析和组装，核心逻辑回到 `src/tdwm/` |
| 实验与方法配置 | `configs/` | 使用现有明确子目录；不得在根目录放散装 YAML |
| 自动化测试与 fixture | `tests/` | 单元测试和集成测试分开；测试临时文件使用 pytest `tmp_path` |
| 设计文档和使用说明 | `docs/` | 只有产生真实文档时才创建该目录 |
| 轻量研究总结和经确认保留的结果 | `reports/` | 不得包含原始大日志、模型或数据集 |
| 本地数据集 | `data/` | Git 忽略，不得提交 |
| 训练、评测、checkpoint、视频和运行日志 | `outputs/` | Git 忽略；需要持久化时使用受控外部 artifact 存储 |
| 可重新生成的缓存 | `.cache/` | Git 忽略，代码不得依赖其永久存在 |
| 项目内临时文件 | `tmp/<task-name>/` | 仅在工具必须使用项目相对路径时采用，任务结束后清理 |

本地临时文件优先写入 `${TMPDIR:-/tmp}/tdwm/<task-name>/`。Gemini 环境则遵循
前述挂载和持久化规则：临时文件使用容器临时存储，需保留的离线训练产物写入
`$GEMINI_DATA_OUT`。任何临时目录都不得包含唯一的源码、配置、实验结论或恢复所需
checkpoint。任务结束前必须删除不再需要的临时文件，并确认其没有进入 Git 暂存区。

### 现有代码保护

- 修改前先阅读目标文件、直接调用方、相关测试和配置，确认现有行为及所属模块；
  不得凭文件名猜测后整文件替换。
- 只修改完成当前请求所需的最小文件和最小代码范围。禁止顺带做全仓格式化、批量
  import 排序、无关重命名、目录重组、抽象重写或依赖升级。
- 未经用户明确要求，不得删除、移动或重命名已有文件，不得改变公共 API、配置键、
  checkpoint 格式、数据格式、评测协议或 CLI 行为。确需改变时，必须说明兼容性影响
  并添加相应迁移或回归测试。
- 不得用新实现覆盖尚未理解的已有实现。添加抽象前必须确认能消除实际复杂度，且
  符合仓库既有模式；能局部修复时不得重写整个模块。
- 发现其他人或用户产生的未提交改动时，默认保留。不得 `stash`、reset、checkout、
  amend 或代为提交这些改动；本次提交只能显式暂存自己修改的路径。
- 生成文件必须能追溯到生成命令和事实来源。不得手工编辑 lockfile、自动生成清单
  或由工具维护的文件；应修改其源配置后重新生成。
- 修改后必须运行与变更风险匹配的测试或检查。无法运行时要明确说明原因，不得把
  “未验证”描述成“已完成”。
...
- 所有受版本控制的项目修改都必须与 GitHub 同步，文档和配置修改也不例外。一次
  修改的标准流程如下，任何一步失败都不得跳过：

  1. 运行 `git status --short --branch`，记录当前分支、远程跟踪关系和既有改动；
  2. 运行 `git pull --ff-only` 拉取当前远程跟踪分支，禁止自动 merge、rebase 或
     强制覆盖；若既有改动可能被更新影响，立即停止并报告；
  3. 只编辑当前任务需要的文件，并在编辑前后检查 `git diff -- <path>`；
  4. 运行必要测试、静态检查或配置解析检查；
  5. 使用 `git add -- <明确路径...>` 逐项暂存，禁止使用 `git add .`、
     `git add -A` 或其他可能混入无关文件的宽泛命令；
  6. 提交前检查 `git diff --cached --name-status`、`git diff --cached --stat` 和
     暂存文件大小，确认没有数据、模型、日志、缓存、临时文件或用户改动；
  7. 创建范围单一、说明清楚的 commit，并推送到当前远程跟踪分支；
  8. 推送后再次检查本地分支与远程同步状态。本次 commit 成功出现在远程后，任务
     才能标记完成。

- 若存在未提交改动，必须先识别其来源和范围。与当前任务无关时保留原状并显式排除
  在暂存区之外；与当前任务重叠、无法安全区分，或远程更新可能覆盖它们时，必须
  报告具体文件并等待用户处理。不得擅自 stash、提交、丢弃或改写用户改动。
- 若当前分支没有远程跟踪分支、远程包含分歧、发生合并冲突、认证失败或 push 被
  拒绝，必须报告阻塞；不得改推其他分支、强制推送或改写历史来绕过问题。
