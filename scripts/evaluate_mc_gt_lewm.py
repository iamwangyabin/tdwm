...
from tdwm.evaluation.mc_gt_lewm import evaluate_mc_gt_lewm


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate MC-GT-LeWM with the locked LeWM Cube CEM protocol."
    )
    parser.add_argument(
        "--config",
        default="configs/experiment/mc_gt_lewm_cube_seed3072_o25.yaml",
    )
    parser.add_argument(
        "--dataset",
...
def main() -> None:
    ...
    if output_dir is None:
        output_dir = (
            ...
            / "mc_gt_lewm_cube_seed3072_o25"
        )
    result = evaluate_mc_gt_lewm(
